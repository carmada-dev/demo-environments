...
eval "$(jq -r '@sh "RESOURCEGROUPID=\(.RESOURCEGROUPID) PROJECTNETWORKID=\(.PROJECTNETWORKID) ENVIRONMENTNETWORKID=\(.ENVIRONMENTNETWORKID) PRIVATENETWORKID=\(.PRIVATENETWORKID) DNSZONENAME=\(.DNSZONENAME)"')"
...
[ -z "$PRIVATENETWORKID" ] || NETWORKIDS+=( "$PRIVATENETWORKID" )
